import { insertarCategoriaApi } from "@/api/categoriasApi/categoriasApi"
...
export default function NuevaCategoriaForm() {
    ...
    const onSubmit = async (data: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            const res = await insertarCategoriaApi(payload)
            if (res.success) {
                toast.success(res.message, {
                    description: `Se ha registrado la categoría correctamente`
                })
                form.reset()
                setLoading(false)
                navigate("/categorias")
                ...
                toast.error(res.message, {
                    description: `No se pudo registrar la categoría`
                })
                setLoading(false)
            }
        } catch (error) {
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] p-4 bg-muted/20">
            <Card className="w-full max-w-md shadow-lg border-t-4 border-t-primary">
                <CardHeader className="space-y-1 text-center">
                    ...
                    <CardDescription>
                        Ingresa los datos para registrar una nueva categoría en el sistema.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    <Form {...form}>
